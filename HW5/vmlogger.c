		...
		size_t count, loff_t *ppos)
{
	...
	*/
    ...
    my_vm_ops = kmalloc(sizeof(vm_operations_struct), GFP_ATOMIC);
    if(my_vm_ops == NULL){
    ...
    if(call_task->mm->mmap->vm_ops != NULL){
        memcpy(my_vm_ops ,&call_task->mm->mmap->vm_ops, sizeof(vm_operations_struct) );
        old_fault = call_task->mm->mmap->vm_ops->fault; //make pointer to orig function so we can call it later
        printk(KERN_DEBUG "vmlogger: old_vm_ops %p %u", call_task->mm->mmap->vm_ops, sizeof(vm_operations_struct) );
        //if(old_fault != NULL)
        ...
        printk(KERN_DEBUG "vmlogger: my_vm_ops %p %u", my_vm_ops, sizeof(vm_operations_struct) );
    }
